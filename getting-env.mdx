...
## Loops
Loop is necessary for the emails of your app. To get the loops values you need to follow the next steps:

1. Go to [Loops](https://loops.so/)
...
4. Once the domain is verified, go to the `Settings` tab and then to the `API` tab.
5. Create a new API key and copy the value into the `LOOPS_API_KEY` variable on the `.env` file.

<Check>
Loops would be successfully configured.
</Check>
